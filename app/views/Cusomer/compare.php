    ...
    <div class="main-content">
        ...
        <?php elseif(empty($vehicles)): ?>
            <div class="card">
                <div class="card-body">
                    <div style="text-align: center; padding: 40px;">
                        <h4>No vehicles to compare. <a href="/customer/vehicles">Browse vehicles</a> and add them to compare!</h4>
                    </div>
                </div>
            </div>
        <?php else: ?>
                        ...
                        <?php foreach($vehicles as $vehicle): ?>
                            <div style="border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 20px; text-align: center;">
                                ...
                                <div style="display: flex; gap: 10px; flex-direction: column;">
                                    <a href="/customer/book-vehicle?id=<?= $vehicle['vehicle_id']; ?>" class="btn btn-primary">Book Now</a>
                                    <a href="/customer/compare?remove=<?= $vehicle['vehicle_id']; ?>" class="btn btn-danger" onclick="return confirm('Remove from comparison?')">Remove</a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
